...
import { COLOR_DEFS, COLOR_BY_KEY, colorMetrics, profileToFractions } from './color-profile.js';
...
async function loadTaggedImages() {
  ...
  _cachePromise = (async () => {
    const { data, error } = await sb
      ...
      .select('id,media_url,media_type,thumb_url,ai_tags,colors')
...
// ============================================================================
// Farbsuche — nutzt vorberechnete Farbprofile aus der DB (Spalte `colors`)
// ----------------------------------------------------------------------------
// Jedes Bild trägt sein Farbprofil (12 Promille-Buckets) bereits in der DB
// (upload-seitig gefüllt + einmaliger Owner-Backfill, s. js/color-profile.js und
// db/color_profiles.sql). Die Suche ist damit ein reiner In-Memory-Filter über
// die ohnehin geladenen Items: KEIN Bild-Download pro Suche, KEINE API-Calls.
// Eine Farbe „zählt" nur, wenn sie das Bild wirklich prägt (Dominanz-/Qualitäts-
// kontrolle in colorMetrics). „blau" → nur Bilder, die klar blau sind.
// ============================================================================

// Fallback für (noch) nicht analysierte Bilder (colors == null, z. B. bevor der
// Owner-Backfill lief oder bei extern verlinkten Medien): wenigstens die ai_tags
// nach dem Farbwort durchsuchen. Weniger präzise als die Pixel-Dominanz, aber die
// Farbsuche liefert dann trotzdem etwas – ohne jeden Bild-Download.
...
function initMoodChat() {
  ...
  // Farben werden nicht mehr als Kacheln angeboten – man tippt sie einfach in
  ...
  // Das Farbprofil jedes Bildes liegt vorberechnet in der DB (Spalte `colors`),
  // wird also mit loadTaggedImages ohnehin schon mitgeladen. Die Suche ist damit
  // ein reiner In-Memory-Filter: KEINE Bild-Downloads, KEINE Analyse zur Laufzeit,
  // KEINE API-Calls → sofort da. Nur IDs gehen ans Grid, sodass der Pill-Shuffle
  // (wie bei der übrigen Chat-Suche) allein die Auswahl mischt.
  async function runColorSearch(colorKeys, label) {
    ...
    // Bewertung anhand der gespeicherten Profile: bestes Match unter den
    // gewünschten Farben, nach Dominanz sortiert. Bilder ohne Profil (colors ==
    // null, noch nicht per Backfill analysiert) sammeln wir separat für den
    // ai_tags-Fallback, damit die Suche auch vor dem Backfill schon etwas liefert.
    const scored = [];
    const unprofiled = [];
    for (const it of imgs) {
      if (!Array.isArray(it.colors) || !it.colors.length) { unprofiled.push(it); continue; }
      const f = profileToFractions(it.colors);
      let best = null;
    ...
    // Für noch nicht analysierte Bilder wenigstens den Tag-Fallback anhängen
    // (dahinter, da unpräziser). Nach vollständigem Backfill greift das nie mehr.
    if (unprofiled.length) ids = ids.concat(tagColorMatch(colorKeys, unprofiled));
